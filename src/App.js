...
import './assets/img/ant-design_bars-outlined.svg';
import { useDispatch, useSelector } from "react-redux";

import { Route, useHistory, useLocation } from 'react-router-dom';

import { getAllTodos, getAllColors, setTodoItem, setTaskItem, changeTaskTitleName, deleteTaskItem, setCompleteToggle, deleteTodoItem } from './redux/actions/Todos';

import AddTodo from "./AddTodo/AddTodo";
import TodoItem from "./TodoItems/TodoItem";
import TaskTable from "./taskTable/TaskTable";
import TaskPlaceholder from './taskTable/TaskPlaceholder';
...
function App() {
  ...
  const [activeTaskObj, setActiveTaskObj] = React.useState(null)

  const {todos, colors, isLoaded} = useSelector(({Todos}) => {
    return {
      todos: Todos.todos,
      colors: Todos.colors,
      isLoaded: Todos.isLoaded
    }
  })

  React.useEffect(() => {
    const activeTaskId = parseInt(location.pathname.split('lists/')[1]);
    const activeTaskNew = todos.filter(item => item.id === activeTaskId);
    setActiveTaskObj(...activeTaskNew)
  }, [isLoaded, location.pathname, todos])
  ...
  const setActiveTask = (obj) => {
    setActiveTaskObj(obj)
  }

  const onAddTask = (obj) => {
    dispatch(setTaskItem(obj))
  }

  const changeTitleTask = (obj) => {
    dispatch(changeTaskTitleName(obj))
  }

  const deleteTask = (id, listId) => {
    dispatch(deleteTaskItem({id, listId}))
  }

  const setCompleteTask = (obj) => {
    dispatch(setCompleteToggle(obj))
  }
      ...
      <div className="todo__container">
        <aside className="todo__sidebar">
          <div className="sidebar__title">
            <p className="sidebar__title-item">MY TODOLIST</p>
          </div>

          <div className="todo__alltodos">
            <TodoItem
              setActiveTask={(item) => {
                history.push(`/`);
              }}
            ...
            {isLoaded && (
              <TodoItem
                ...
                trashTodo={deleteTodo}
                items={todos.map((todo) => {
                  todo.color = colors.filter((color) => color.id === todo.colorId)[0];
                  return todo;
                })}
              />
            )}

            {colors && isLoaded && <AddTodo onAddTodo={onAddTodo} colors={colors} />}
          </div>
        </aside>
        <div className="todo__alltasktables">
          <Route path="/" exact>
