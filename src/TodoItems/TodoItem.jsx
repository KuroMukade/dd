...
function TodoItem({ items, colors, onOpenPopup, trashTodo, setActiveTask }) {
  const deleteTodo = (id) => {
  ...
  return (
    <div onClick={onOpenPopup} className="todo__list-wrapper">
      {items.map((item) => (
        <div
          onClick={setActiveTask ? () => setActiveTask(item) : null}
          key={`item-${item.id}`}
          className={item.img ? 'todo__list-item' : 'todo__list-item todo-item'}>
          {item.img && item.img}
          <div style={{ color: item.color && item.color.hex }} className="todo__list-text">
            {item.name}
          </div>
          {!item.img && (
            <div className="todo__list-delete" onClick={() => deleteTodo(item.id)}>
              <svg
                width="16"
                height="16"
                viewBox="0 0 16 16"
